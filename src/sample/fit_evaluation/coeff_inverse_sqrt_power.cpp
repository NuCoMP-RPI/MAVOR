...
double inverse_sqrt_power_alpha_recurrence__(double const & x, int const & k){
    return 1.0/sqrt(x);
}

double inverse_sqrt_power_beta_recurrence__(double const & x, int const & k){
    return 0;
}

double eval_inverse_sqrt_power_order__(double const & x, int const order){
...
double naive_inverse_sqrt_power_vec(double const & x, std::vector<double> const& coeffs){
    ...
    for (int i = 0; i < coeffs.size(); ++i){
        val += coeffs[i]*eval_inverse_sqrt_power_order__(x, i);
    }
    return val;
}
...
double clenshaw_inverse_sqrt_power_general_vec(double const & x, std::vector<double> const & coeffs){
    return clenshaw(x, coeffs, inverse_sqrt_power_alpha_recurrence__, inverse_sqrt_power_beta_recurrence__, eval_inverse_sqrt_power_order__);
}

double clenshaw_inverse_sqrt_power_general_iter(double const & x, Iter begin, Iter end) {
    return clenshaw(x, begin, end, inverse_sqrt_power_alpha_recurrence__, inverse_sqrt_power_beta_recurrence__, eval_inverse_sqrt_power_order__);
}