...
int main(int argc, char* argv[]){
    ...
    auto &cdf_grids = *sab.add_option_group("CDF Grid Settings", "Sets the behavior to use/generate storage CDF grids for the sampling distributions.  Sigmoid CDFs is the default behavior.");
    cdf_grids.require_option(0,1);

    auto linearize_cdfs_flag = cdf_grids.add_flag("--linearize", linearize_cdfs, "Initialize CDF grids to extrema values and perform a linearization process.  NOT SUITABLE FOR OTF DATA.");

    auto sigmoid_cdfs_flag = cdf_grids.add_flag("--sigmoid", use_sigmoid_cdfs, "Initialize CDF grids to sigmoid functions. No linearization is performed. SUITABLE FOR OTF.");
    ...
    auto external_cdfs_flag = cdf_grids.add_flag("--user_cdf_grids", use_external_cdfs, "Initialize CDF grids to user defined CDF files. No linearization is performed. SUITABLE FOR OTF.");
    sab.add_option("--alpha_cdf_grid", alpha_cdf_grid_loc, "Sets the location of the alpha cdf grid file to use.");
    sab.add_option("--beta_cdf_grid", beta_cdf_grid_loc, "Sets the location of the beta cdf grid file to use.");
    ...
    // elastic subcommand
    CLI::App &elastic = *mavor.add_subcommand("elastic", "Copies an OTF coefficient file generated by MAVOR and adds elastic scattering data.");
    elastic.add_option("-i,--otf_file", elastic_otf_file, "Sets the OTF coefficient file to copy and where the elastic data will be added.");
    elastic.add_option("-e,--endf_file", elastic_endf_file, "Sets the ENDF file where the elastic data will be obtained.");
    elastic.add_option("-o,--full_otf_file", elastic_full_otf_file, "Set the final file path that will contain all the information to sample thermal scattering data OTF.");
    ...
    if (sab.parsed()){
        ...
        if (*linearize_cdfs_flag){
            use_external_cdfs = false;
            use_sigmoid_cdfs = false;
        }
        if (*sigmoid_cdfs_flag){
            use_external_cdfs = false;
            linearize_cdfs = false;
        }
        if (*external_cdfs_flag){
            use_sigmoid_cdfs = false;
            linearize_cdfs = false;
        }
        run_sab();
